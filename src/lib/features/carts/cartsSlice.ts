import { compareArrays } from "@/lib/hooks/utils";
  ...
  reducers: {
    addToCart: (state: any , action: PayloadAction<CartItem>) => {
      ...
      if (isItemInCart) {
        state.cart = {
          ...state.cart,
          items: state.cart.items.map((eachCartItem : any) => {
            if (
    ...
    removeCartItem: (state :any, action: PayloadAction<RemoveCartItem>) => {
    ...
    remove: (
      ...
      state.cart = {
        ...state.cart,
        items: state.cart.items.filter((pItem : any) => {
